...
for (const name of ['*', ...AI_AGENTS]) {
  ...
  assert(
    g.disallow.length === STAFF_DISALLOW.length &&
      STAFF_DISALLOW.every((p, i) => g.disallow[i] === p),
    name + ' keeps the locked staff Disallow list'
  );
  assert(
    !g.disallow.some((p) => p === '/' || p === '/guides/' || p === '/policies/'),
    name + ' does not block public docs'
  );
}

assert(
  sitemaps.includes('https://docs.unitedmobilerv.com/sitemap.xml'),
  'Sitemap is the custom-domain sitemap'
);
assert(!sitemaps.some((s) => /pages\.dev/.test(s)), 'Sitemap is not the pages.dev host');
